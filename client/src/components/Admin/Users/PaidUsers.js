...
class PaidUsers extends Component {
  ...
  genTableRows(users) {
    ...
    for (const key in users) {
      const plans = [];
      const dateOfBuy = [];
      for(const pkey in users[key].plans) {
        plans.push(<span style ={{overflow: 'visible'}}>{users[key].plans[pkey].description} <br />  </span>)
        dateOfBuy.push(<span style ={{overflow: 'visible'}}>{users[key].plans[pkey].date} <br />  </span>)
      }
      tableValues.push(
        <div className={styles.usersTableRow} key={Math.random(123)} style = {{paddingRight: 0}}>
          ...
          <span style={{fontFamily: "G", display: 'flex', flexFlow: 'column', width: '400px', overflow: 'visible'}}>
            {plans}
          </span>
          <span style={{fontFamily: "G", display: 'flex', flexFlow: 'column', width: '400px', overflow: 'visible'}}>
            {dateOfBuy}
  ...
  async componentDidMount() {
    ...
    const users = await this.props.getPaidUsers();
          ...
          <div className={styles.content}>
            ...
            <div className={styles.search}>
                ...
                <div className={styles.usersTable}>
                  <div className={styles.paidUsersTableHeader}>
                    <span>
                      Name{" "}
...
export default connect(mapStateToProps, actions)(PaidUsers);
